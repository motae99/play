...
export default class MapViewScreen extends Component {
  ...
  componentDidMount() {
    this.animation.addListener(({value}) => {
      ...
      if (index >= this.context.documentData.length) {
        index = this.context.documentData.length - 1;
      }
      if (index <= 0) {
        index = 0;
      }
      ...
      this.regionTimeout = setTimeout(() => {
        if (this.index !== index) {
          this.index = index;
          const {coordinate} = this.context.documentData[index];
  ...
  getCoordsFromName(loc) {
    ...
    this.context.locationQuery(range);
  ...
  render() {
    const {documentData} = this.context;
