...
const AgendaScreen = () => {
  ...
  const booking = () => {
    this.props.selected({
      data: { date: this.state.date, time: this.state.time }
    });
  };
    ...
    <View style={{ flex: 1 }}>
      <Agenda
        ...
        renderItem={(item, firstItemInDay) => {
          return (
            <CalenderItem parentCallback={this.callbackFunction} data={item} />
          );
        }}
      ...
      <View style={styles.footer}>
        {this.state.date && this.state.time ? (
          <View style={styles.bottomButtons}>
            <Text>
              Selected Date: {this.state.date} at {this.state.time}
            </Text>
            <TouchableOpacity
